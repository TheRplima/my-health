...
const ConsumoAguaHoje = (token) => {
    ...
    const { userProfileData, setUserProfileData,waterIngestedToday } = useUserProfileData()
    
    const handleRegisterWaterIngestion = async (e) => {
        const ret = await RegisterWaterIngestion(amount,token.token)
        setUserProfileData(userProfileData,ret.waterIngestion)
    }
    return (
        <>
            <Card className="mb-3">
                <Card.Header className='d-flex'>
                    ...
                    <RegisterWaterIngestionModal userData={userProfileData} handleRegisterWaterIngestion={handleRegisterWaterIngestion} setAmount={setAmount} />
                </Card.Header>
                <Card.Body>
                    <Card.Subtitle className="mb-3 text-muted"><strong>Total consumido hoje:</strong> {waterIngestedToday} ml</Card.Subtitle>
                    <ProgressBar animated now={waterIngestedToday} max={userProfileData.daily_water_amount} label={`${(waterIngestedToday / userProfileData.daily_water_amount) * 100}%`} />
                    <table className="table table-hover">
                        ...
                        <tbody>
                            {userProfileData.water_ingestion_today.map((waterIngestion, index) => {
                                return (
                                    <tr key={index}>
